...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            action="append",
            default=[],
            help="Repeatable Twitter/X username(s) to scan for IOC text.",
        )
        parser.add_argument("--twitter-limit", type=int, default=50)
    ...
    def handle(self, *args, **options):
        ...
        for username in options["twitter_user"]:
            twitter_result = ingest_twitter_user(
                username=username,
                limit=options["twitter_limit"],
            )
            result["twitter"].append(twitter_result)
